...
def main(
    ...
    mix:   Param("Type of mix, alpha", tuple)=('input', 0.2),
    ...
    for run in range(runs):
        ...
        # Original code: cbs = MixUp(mixup) if mixup else []

        # Code added begin
        try:
          if mix[0] == 'input': 
            cbs = MixUp(mix[1])
            print(f'Using mixup with alpha = {mix[1]}')
          elif mix[0] == 'manifold': 
            cbs = ManifoldMixup(mix[1])
            print(f'Using manifold mixup with alpha = {mix[1]}')
          elif mix[0] == 'cut': 
            cbs = CutMix(mix[1])
            print(f'Using cutmix with alpha = {mix[1]}')
          else : 
            cbs = []
            print('No mix will be used')
        except:
          cbs = []
          print('No mix will be used')
        # Code added end
